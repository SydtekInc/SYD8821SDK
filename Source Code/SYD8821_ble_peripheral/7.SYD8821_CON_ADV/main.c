...
static bool coex_adv_flag = false;
...
{	
	...
	#endif
	ble_SetTxPower(BLE_TX_POWER_2_DBM);
	while(1)
	{				
		ble_sched_execute(); 
        //gpo_toggle(LED2_Pin);
...
//		}
		...
		if(connect_flag == 1 && coex_adv_flag == false)
		{
			...
			gap_s_ble_address_get(&dev_addr);
			gap_s_coex_adv_data_set(ADV_NOCONN_IND, ADV_DATA, sizeof(ADV_DATA), dev_addr.addr, dev_addr.type);

	...
	switch(target){
		...
		case 1:
				if((link_app.latency <0x000A) && (link_app.interval <0x0050)){
					...
					smart_params.updatelatency=15;//0x000A;
...
{	
	...
	sys_mcu_clock_set(MCU_CLOCK_20_MHZ);
	// RC bumping
	...
	#endif
//	ble_SetTxPower(BLE_TX_POWER_4_DBM);
	gpio_init();								 //gpio初始化
	...
	{				
		...
		if(start_tx==0)
		{
			...
			//SystemSleep(POWER_SAVING_RC_OFF, BF_MODULE, 11000 , (PMU_WAKEUP_CONFIG_TYPE)(TIMER_WAKE_EN));
		}
		#endif
	}	
}

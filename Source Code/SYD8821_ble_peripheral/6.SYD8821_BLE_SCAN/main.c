...
void ble_evt_callback(struct gap_ble_evt *p_evt)
{
	...
	else if(p_evt->evt_code == GAP_EVT_ADV_REPORT)
	{		
		#if defined(CONFIG_UART_ENABLE) || defined(_SYD_RTT_DEBUG_)
		DBGPRINTF(("adv_addr_type:%x",p_evt->evt.advertising_report_evt.peer_dev_addr.type));
		DBGHEXDUMP("adv_addr:",p_evt->evt.advertising_report_evt.peer_dev_addr.addr,BD_ADDR_SZ);
		DBGPRINTF(("adv_rssi:%x\r\n",p_evt->evt.advertising_report_evt.rssi));
		DBGPRINTF(("adv_data_len:%x\r\n",p_evt->evt.advertising_report_evt.len));
		DBGHEXDUMP("adv_data:\r\n",p_evt->evt.advertising_report_evt.buf,p_evt->evt.advertising_report_evt.len);
		#endif
	}
...
{	
	...
	#if defined(CONFIG_UART_ENABLE) || defined(_SYD_RTT_DEBUG_)
		dbg_printf("SYD8821 BLEscan %s:%s\r\n",__DATE__ ,__TIME__);
